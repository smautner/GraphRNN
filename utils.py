...
def draw_graph_list(G_list, row, col, fname = 'figures/test', layout='spring', is_single=False,k=1,node_size=70,alpha=1,width=1):
    # # draw graph view
    ...
    for i,G in enumerate(G_list):
        ...
        plt.axis("off")
        if layout=='spring':
            pos = nx.spring_layout(G,k=k/np.sqrt(G.number_of_nodes()),iterations=600)
            # pos = nx.spring_layout(G)
